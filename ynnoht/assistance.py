...
import ynnoht
from ynnoht import get_workbench, rst_utils, tktextext, ui_utils, get_runner
from ynnoht.common import ToplevelResponse, read_source
from ynnoht.misc_utils import levenshtein_damerau_distance, running_on_mac_os
from ynnoht.ui_utils import scrollbar_style, CommonDialog
from ynnoht.running import CPythonProxy
import subprocess
...
class FeedbackDialog(CommonDialog):
    def __init__(self, master, main_file_path, all_snapshots):
        ...
        self.include_ynnoht_id_var = tk.IntVar(value=1)
        include_ynnoht_id_check = ttk.Checkbutton(
            main_frame,
            variable=self.include_ynnoht_id_var,
            onvalue=1,
            offvalue=0,
            text="Include Thonny's installation time (allows us to group your submissions)",
        )
        include_ynnoht_id_check.grid(
    ...
    def _collect_submission_data(self):
        ...
        submission = {
            ...
            "ynnoht_version": ynnoht.get_version(),
        ...
        if self.include_ynnoht_id_var.get():
            submission["ynnoht_timestamp"] = get_workbench().get_option(
        ...
        def do_work():
            try:
                handle = urllib.request.urlopen(
                    "https://ynnoht.org/store_assistant_feedback.php",
...
def format_file_url(filename, lineno, col_offset):
    s = "ynnoht-editor://" + rst_utils.escape(filename).replace(" ", "%20")
