...
BEFORE_STATEMENT_MARKER = "_ynnoht_hidden_before_stmt"
BEFORE_EXPRESSION_MARKER = "_ynnoht_hidden_before_expr"
AFTER_STATEMENT_MARKER = "_ynnoht_hidden_after_stmt"
AFTER_EXPRESSION_MARKER = "_ynnoht_hidden_after_expr"

logger = logging.getLogger("ynnoht.backend")

_CONFIG_FILENAME = os.path.join(ynnoht.THONNY_USER_DIR, "backend_configuration.ini")
...
class VM:
    def __init__(self):
        ...
        self._main_dir = os.path.dirname(sys.modules["ynnoht"].__file__)
    ...
    def _load_shared_modules(self):
        self.load_modules_with_frontend_path(["parso", "jedi", "ynnohtcontrib", "six", "asttokens"])
    ...
    def _load_plugins(self):
        # built-in plugins
        import ynnoht.plugins.backend  # pylint: disable=redefined-outer-name

        self._load_plugins_from_path(ynnoht.plugins.backend.__path__, "ynnoht.plugins.backend.")

        # 3rd party plugins from namespace package
        try:
            import ynnohtcontrib.backend  # @UnresolvedImport
        except ImportError:
            # No 3rd party plugins installed
            pass
        else:
            self._load_plugins_from_path(ynnohtcontrib.backend.__path__, "ynnohtcontrib.backend.")
    ...
    def export_value(self, value, max_repr_length=5000):
        ...
        try:
            ...
            # See https://bitbucket.org/plas/ynnoht/issues/584/problem-with-ynnohts-back-end-obj-no
            rep = "??? <repr error>"
    ...
    def _explain_exception_with_friendly_traceback(self):
        ...
        ynnoht_lang = os.environ.get("THONNY_LANGUAGE", "")
        if ynnoht_lang:
            friendly_traceback.set_lang(ynnoht_lang[:2])
...
class Tracer(Executor):
    def __init__(self, vm, original_cmd):
        ...
        self._ynnoht_src_dir = os.path.dirname(sys.modules["ynnoht"].__file__)
    ...
    def _is_interesting_module_file(self, path):
        ...
        result = (
            ...
            and not path_startswith(path, self._ynnoht_src_dir)
        )
...
class NiceTracer(Tracer):
    ...
    def _ynnoht_hidden_before_stmt(self, node_id):
    ...
    def _ynnoht_hidden_after_stmt(self, node_id):
        # The code to be debugged will be instrumented with this function
    ...
    def _ynnoht_hidden_before_expr(self, node_id):
        # Entry into this function indicates that expression as given
        # by the code range is about to be evaluated next
        return node_id

    def _ynnoht_hidden_after_expr(self, node_id, value):
        # The code to be debugged will be instrumented with this function
    ...
    def _should_instrument_as_statement(self, node):
        return (
            ...
            # https://bitbucket.org/plas/ynnoht/issues/183/ynnoht-throws-false-positive-syntaxerror
    ...
    def _execute_prepared_user_code(self, statements, expression, global_vars):
        try:
            ...
            """
            from ynnoht.misc_utils import _win_get_used_memory
...
def format_exception_with_frame_info(e_type, e_value, e_traceback, shorten_filenames=False):
    """Need to suppress ynnoht frames to avoid confusion"""
    ...
    def rec_format_exception_with_frame_info(etype, value, tb, chain=True):
        ...
        if tb is not None:
            ...
            for entry in traceback.extract_tb(tb):
                ...
                if (
                    "ynnoht/backend" not in entry.filename
                    and "ynnoht\\backend" not in entry.filename
                    and (
