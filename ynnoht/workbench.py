...
from ynnoht.code import EditorNotebook
from ynnoht.common import Record, UserError, normpath_with_actual_case
from ynnoht.config import try_load_configuration
from ynnoht.config_ui import ConfigurationDialog
from ynnoht.misc_utils import (
    running_on_linux,
    running_on_mac_os,
    running_on_windows,
    running_on_rpi,
...
from ynnoht.running import BackendProxy, Runner
from ynnoht.shell import ShellView
from ynnoht.ui_utils import (
    AutomaticNotebook,
...
class Workbench(tk.Tk):
    ...
    def _init_configuration(self) -> None:
        self._configuration_manager = try_load_configuration(ynnoht.CONFIGURATION_FILE)
        self._configuration_pages = []  # type: List[Tuple[str, str, Type[tk.Widget]]

        self.set_default("general.single_instance", ynnoht.SINGLE_INSTANCE_DEFAULT)
    ...
    def _init_language(self) -> None:
        ...
        if language_code in languages.LANGUAGES_DICT:
            ...
            language = gettext.translation("ynnoht", path, [language_code])
            language.install()
    ...
    def _init_icon(self) -> None:
        ...
        if running_on_linux() and ui_utils.get_tk_version_info() >= (8, 6):
            self.iconphoto(True, self.get_image("ynnoht.png"))
        else:
            icon_file = os.path.join(self.get_package_dir(), "res", "ynnoht.ico")
            try:
    ...
    def _load_plugins(self) -> None:
        # built-in plugins
        import ynnoht.plugins  # pylint: disable=redefined-outer-name

        self._load_plugins_from_path(
            ynnoht.plugins.__path__, "ynnoht.plugins."  # type: ignore
        )

        # 3rd party plugins from namespace package
        try:
            import ynnohtcontrib  # @UnresolvedImport
        except ImportError:
            # No 3rd party plugins installed
            pass
        else:
            self._load_plugins_from_path(ynnohtcontrib.__path__, "ynnohtcontrib.")
    ...
    def _add_main_backends(self) -> None:
        ...
        from ynnoht import running_config_page
    ...
    def _check_init_server_loop(self) -> None:
        ...
        if not self.get_option("general.single_instance") or os.path.exists(ynnoht.IPC_FILE):
    ...
    def _create_server_socket(self):
        if running_on_windows():
            ...
            with open(ynnoht.IPC_FILE, "w") as fp:
                fp.write(str(port) + "\n")
                fp.write(secret + "\n")

        else:
            server_socket = socket.socket(socket.AF_UNIX)  # @UndefinedVariable
            server_socket.bind(ynnoht.IPC_FILE)
            secret = ""

        os.chmod(ynnoht.IPC_FILE, 0o600)
    ...
    def update_image_mapping(self, mapping: Dict[str, str]) -> None:
        """Was used by ynnoht-pi. Not recommended anymore"""
    ...
    def get_package_dir(self):
        """Returns ynnoht package directory"""
        return os.path.dirname(sys.modules["ynnoht"].__file__)
    ...
    def unbind(self, sequence: str, func=None) -> None:
        if sequence.startswith("<"):
            ...
            try:
                ...
                logging.getLogger("ynnoht").exception(
        ...
        if self._toolbar.winfo_ismapped():
            for group_frame in self._toolbar.grid_slaves(0):
                for button in group_frame.pack_slaves():
                    if ynnoht._runner is None or button.tester and not button.tester():
    ...
    def _guard_font_size(self, size: int) -> int:
        # https://bitbucket.org/plas/ynnoht/issues/164/negative-font-size-crashes-ynnoht
        MIN_SIZE = 4
        MAX_SIZE = 200
        if size < MIN_SIZE:
            return MIN_SIZE
    ...
    def destroy(self) -> None:
        try:
            if self._is_server() and os.path.exists(ynnoht.IPC_FILE):
                os.remove(ynnoht.IPC_FILE)
    ...
    def open_url(self, url):
        m = re.match(r"^ynnoht-editor://(.*?)(#(\d+)(:(\d+))?)?$", url)
        if m is not None:
