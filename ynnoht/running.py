...
from ynnoht import THONNY_USER_DIR, common, get_runner, get_shell, get_workbench, ui_utils
from ynnoht.code import get_current_breakpoints, get_saved_current_script_filename, is_remote_path
...
from ynnoht.misc_utils import construct_cmd_line, running_on_mac_os, running_on_windows

from typing import Any, List, Optional, Sequence, Set  # @UnusedImport; @UnusedImport
from ynnoht.terminal import run_in_terminal
from ynnoht.ui_utils import select_sequence, show_dialog, CommonDialogEx
...
class Runner:
    ...
    def start(self) -> None:
        ...
        try:
            ...
            logging.getLogger("ynnoht").exception("Problem allocating console")
            _console_allocated = False
    ...
    def _init_commands(self) -> None:
        ...
        try:
            import ynnoht.plugins.debugger  # @UnusedImport
    ...
    def _check_alloc_console(self) -> None:
        if sys.executable.endswith("pythonw.exe"):
            ...
            try:
                ...
                # See https://bitbucket.org/plas/ynnoht/issues/508/cant-allocate-windows-console-when
                # Without flush the console window becomes visible, but Thonny can be still used
                logging.getLogger("ynnoht").exception("Problem with finalizing console allocation")
...
class SubprocessProxy(BackendProxy):
    ...
    def _start_background_process(self, clean=None):
        ...
        # see https://github.com/ynnoht/ynnoht/issues/808
        ...
        while self.process_is_alive():
            ...
            if data == "":
                ...
                try:
                    ...
                    # then the suffix can be ynnoht message
...
class CPythonProxy(SubprocessProxy):
    ...
    def _get_launcher_with_args(self):
        import ynnoht.backend_launcher

        return [ynnoht.backend_launcher.__file__]
    ...
    def _loop_gui_update(self, force=False):
        if force or get_runner().is_waiting_toplevel_command():
            try:
                ...
                # https://github.com/ynnoht/ynnoht/issues/966
                logging.getLogger("ynnoht").exception("Could not send process_gui_events")
...
class PrivateVenvCPythonProxy(CPythonProxy):
    ...
    def _create_private_venv(self, path, description, clear=False, upgrade=False):
        ...
        # it tries to link venv to ynnoht.exe.
        ...
        from ynnoht.ui_utils import SubprocessDialog
...
def is_bundled_python(executable):
    return os.path.exists(os.path.join(os.path.dirname(executable), "ynnoht_python.ini"))
