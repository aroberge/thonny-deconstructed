...
def _compute_ynnoht_user_dir():
    if os.environ.get("THONNY_USER_DIR", ""):
        ...
        return os.path.join(sys.prefix, ".ynnoht")
    elif platform.system() == "Windows":
        from ynnoht import misc_utils
...
def is_portable():
    ...
    portable_marker_path = os.path.join(os.path.dirname(sys.executable), "portable_ynnoht.ini")
    shared_marker_path = os.path.join(os.path.dirname(sys.executable), "shared_ynnoht.ini")
...
def get_version():
    try:
        package_dir = os.path.dirname(sys.modules["ynnoht"].__file__)
...
THONNY_USER_DIR = _compute_ynnoht_user_dir()
...
def _get_ipc_file_path():
    ...
    ipc_dir = os.path.join(base_dir, "ynnoht-%s" % getpass.getuser())
...
def _check_welcome():
    ...
    if not os.path.exists(CONFIGURATION_FILE) and not misc_utils.running_on_rpi():
        from ynnoht.first_run import FirstRunWindow
        from ynnoht.config import ConfigurationManager
...
if TYPE_CHECKING:
    ...
    import ynnoht.workbench
    from ynnoht.workbench import Workbench
    from ynnoht.running import Runner
