...
import ynnoht.memory
from ynnoht import get_runner, get_workbench, ui_utils
from ynnoht.common import InlineCommand
from ynnoht.misc_utils import shorten_repr
...
class ObjectInspector(ttk.Frame):
    ...
    def _show_object_by_id(self, object_id, via_navigation=False):
        ...
        if self.winfo_ismapped() and self.object_id != object_id:
            ...
            self._set_title("object @ " + ynnoht.memory.format_object_id(object_id))
            self.request_object_info()
    ...
    def set_object_info(self, object_info):
        ...
        if object_info is None or "error" in object_info:
            ...
            self._set_title(
                ...
                + " @ "
                + ynnoht.memory.format_object_id(object_info["id"])
            )
...
class ElementsInspector(ynnoht.memory.MemoryFrame, ContentInspector):
    def __init__(self, master):
        ContentInspector.__init__(self, master)
        ynnoht.memory.MemoryFrame.__init__(self, master, ("index", "id", "value"))
    ...
    def set_object_info(self, object_info):
        ...
        for element in object_info["elements"]:
            ...
            self.tree.set(node_id, "id", ynnoht.memory.format_object_id(element.id))
            self.tree.set(
                node_id, "value", shorten_repr(element.repr, ynnoht.memory.MAX_REPR_LENGTH_IN_GRID)
            )
            index += 1
...
class DictInspector(ynnoht.memory.MemoryFrame, ContentInspector):
    def __init__(self, master):
        ContentInspector.__init__(self, master)
        ynnoht.memory.MemoryFrame.__init__(self, master, ("key_id", "id", "key", "value"))
    ...
    def set_object_info(self, object_info):
        ...
        for key, value in object_info["entries"]:
            node_id = self.tree.insert("", "end")
            self.tree.set(node_id, "key_id", ynnoht.memory.format_object_id(key.id))
            self.tree.set(
                node_id, "key", shorten_repr(key.repr, ynnoht.memory.MAX_REPR_LENGTH_IN_GRID)
            )
            self.tree.set(node_id, "id", ynnoht.memory.format_object_id(value.id))
            self.tree.set(
                node_id, "value", shorten_repr(value.repr, ynnoht.memory.MAX_REPR_LENGTH_IN_GRID)
            )
...
class AttributesFrame(ynnoht.memory.VariablesFrame):
    def __init__(self, master):
        ynnoht.memory.VariablesFrame.__init__(self, master)
