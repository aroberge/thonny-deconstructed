...
import traceback
import queue
from ynnoht.plugins.micropython.connection import (
...
import os
import time
from ynnoht.misc_utils import find_volumes_by_name, sizeof_fmt
import jedi
import io
import tokenize
from ynnoht.running import EXPECTED_TERMINATION_CODE
import binascii
import shutil
...
# Output tokens
THONNY_MSG_START = b"\x02<ynnoht>"
THONNY_MSG_END = b"</ynnoht>\x04"
EOT = b"\x04"
NORMAL_PROMPT = b">>> "
LF = b"\n"
OK = b"OK"
...
logger = logging.getLogger("ynnoht.micropython.backend")
...
class MicroPythonBackend:
    ...
    def _fetch_uname(self):
        res = self._evaluate("__ynnoht_os.uname()", prelude="import os as __ynnoht_os")
        return {
            "sysname": res[0],
            "nodename": res[1],
            "release": res[2],
    ...
    def _fetch_cwd(self):
        return self._evaluate(
            "__ynnoht_os.getcwd() if hasattr(__ynnoht_os, 'getcwd') else ''",
            prelude="import os as __ynnoht_os",
        )
    ...
    def _process_until_raw_prompt(self, capture_output=False):
        ...
        In the end of {stdout} there may be \x02{value-for-ynnoht}
    ...
    def _cmd_cd(self, cmd):
        if len(cmd.args) == 1:
            ...
            path = cmd.args[0]
            self._execute("import os as __ynnoht_os; __ynnoht_os.chdir(%r)" % path)
            self._cwd = self._fetch_cwd()
            return {}
        else:
    ...
    def _read_file(self, path):
        ...
        self._execute_without_output("__ynnoht_fp = open(%r, 'rb')" % path)
        ...
        while True:
            ...
            if "binascii" in self._builtin_modules:
                block = binascii.unhexlify(
                    self._evaluate("__temp_hexlify(__ynnoht_fp.read(%s))" % block_size)
                )
            else:
                block = self._evaluate("__ynnoht_fp.read(%s)" % block_size)
            if block:
                yield block
    ...
    def _write_file_via_serial(self, content_blocks, target_path, notifier=None):
        ...
        try:
            _, err, _ = self._execute(
                ...
                __ynnoht_path = '{path}'
                __ynnoht_written = 0
                __ynnoht_fp = open(__ynnoht_path, 'wb')
                """
            ...
            if "binascii" in self._builtin_modules:
                self._execute_without_output(
                    dedent(
                        """
                    from binascii import unhexlify as __ynnoht_unhex
                    def __W(x):
                        global __ynnoht_written
                        __ynnoht_written += __ynnoht_fp.write(__ynnoht_unhex(x))
                        __ynnoht_fp.flush()
                """
                    )
                )
                ...
                self._execute_without_output(
                    ...
                    def __W(x):
                        global __ynnoht_written
                        __ynnoht_written += __ynnoht_fp.write(x)
                """
                    )
                )
            ...
            bytes_received = self._evaluate("__ynnoht_written")
    ...
    def _sync_all_filesystems(self):
        self._execute_without_output(
            dedent(
                """
            try:
                from os import sync as __ynnoht_sync
                __ynnoht_sync()
                del __ynnoht_sync
            except ImportError:
                pass
    ...
    def _list_remote_files_with_info(self, paths):
        ...
        self._execute_without_output(
            dedent(
                """
            try:
                import os as __ynnoht_os
                from os import stat as __ynnoht_stat

                def __ynnoht_getsize(path):
                    return __ynnoht_stat(path)[6]

                def __ynnoht_isdir(path):
                    return __ynnoht_stat(path)[0] & 0o170000 == 0o040000

            except ImportError:
                __ynnoht_stat = None
                # micro:bit
                from os import size as __ynnoht_getsize
        ...
        self._execute_without_output(
            dedent(
                """
            def __ynnoht_rec_list_with_size(path):
                result = {}
                if __ynnoht_isdir(path):
                    for name in __ynnoht_os.listdir(path):
                        result.update(__ynnoht_rec_list_with_size(path + "/" + name))
                else:
                    result[path] = __ynnoht_getsize(path)
        ...
        result = []
        for requested_path in paths:
            sizes = self._evaluate("__ynnoht_rec_list_with_size(%r)" % requested_path)
            for path in sizes:
                result.append(
                    {
                        "path": path,
        ...
        self._execute_without_output(
            dedent(
                ...
                del __ynnoht_os
                del __ynnoht_stat
                del __ynnoht_getsize
                del __ynnoht_isdir
                del __ynnoht_rec_list_with_size
            """
            )
        )
        return result
    ...
    def _get_existing_remote_files(self, paths):
        ...
        return self._evaluate(
            ...
            prelude=dedent(
                ...
                import os as __ynnoht_os
                __ynnoht_result = []
                for __ynnoht_path in %r:
                    try:
                        __ynnoht_os.%s(__ynnoht_path)
                        __ynnoht_result.append(__ynnoht_path)
                    except OSError:
                        pass
                """
            )
    ...
    def _get_file_size(self, path):
        if self._supports_directories():
            script = "__ynnoht_os.stat(%r)[6]"
        else:
            script = "os.stat(%r)[6]"

        return self._evaluate(script % path, prelude="import os as __ynnoht_os")
    ...
    def _makedirs_via_serial(self, path):
        ...
        script = (
            dedent(
                """
            import os as __ynnoht_os
            __ynnoht_parts = %r.split('/')
            for i in range(2, len(__ynnoht_parts) + 1):
                __ynnoht_path = "/".join(__ynnoht_parts[:i])
                try:
                    __ynnoht_os.stat(__ynnoht_path)
                except OSError:
                    # does not exist
                    __ynnoht_os.mkdir(__ynnoht_path)
    ...
    def _delete_via_serial(self, paths):
        if not self._supports_directories():
            self._execute_without_output(
                ...
                import os as __ynnoht_os
                for __ynnoht_path in %r:
                    __ynnoht_os.remove(__ynnoht_path)
            ...
            self._execute_without_output(
                dedent(
                    """
                import os as __ynnoht_os
                def __ynnoht_delete(path):
                    if __ynnoht_os.stat(path)[0] & 0o170000 == 0o040000:
                        for name in __ynnoht_os.listdir(path):
                            child_path = path + "/" + name
                            __ynnoht_delete(child_path)
                        __ynnoht_os.rmdir(path)
                    else:
                        __ynnoht_os.remove(path)

                for __ynnoht_path in %r:
                    __ynnoht_delete(__ynnoht_path)
    ...
    def _get_fs_mount_label(self):
        ...
        result = self._evaluate(
            ...
            prelude=dedent(
                """
            try:
                from storage import getmount as __ynnoht_getmount
                try:
                    __ynnoht_result = __ynnoht_getmount("/").label
                finally:
                    del __ynnoht_getmount
            except ImportError:
                __ynnoht_result = None
            except OSError:
                __ynnoht_result = None
        """
            ),
            cleanup="del __ynnoht_result",
        )
    ...
    def _get_fs_info(self, path):
        result = self._evaluate(
            dedent(
                """{
                    "total" : __ynnoht_total,
                    "used" : __ynnoht_used,
                    "free": __ynnoht_free,
                    "sizes": __ynnoht_sizes
                }"""
            ),
            prelude=dedent(
                """
                try:
                    from os import statvfs as __ynnoht_statvfs
                    __ynnoht_stat = __ynnoht_statvfs(%r)
                    __ynnoht_total = __ynnoht_stat[2] * __ynnoht_stat[0]
                    __ynnoht_free = __ynnoht_stat[3] * __ynnoht_stat[0]
                    __ynnoht_used = __ynnoht_total - __ynnoht_free
                    __ynnoht_sizes = None
                    del __ynnoht_statvfs
                    del __ynnoht_stat
                except ImportError:
                    import os as __ynnoht_os
                    __ynnoht_sizes = [__ynnoht_os.size(name) for name in __ynnoht_os.listdir()]
                    __ynnoht_used = None
                    __ynnoht_total = None
                    __ynnoht_free = None
                    del __ynnoht_os
    ...
    def _get_microbit_file_sizes(self):
        return self._evaluate(
            "{name : __ynnoht_os.size(name) for name in __ynnoht_os.listdir()}",
            prelude="import os as __ynnoht_os",
            cleanup="del __ynnoht_os",
        )
    ...
    def _get_dirs_child_data_generic(self, paths):
        return self._evaluate(
            ...
            prelude=dedent(
                ...
                __ynnoht_path = None
                __ynnoht_st = None
                __ynnoht_child_names = None
                __ynnoht_children = None
                __ynnoht_name = None
                __ynnoht_real_path = None
                __ynnoht_full = None

                for __ynnoht_path in %(paths)r:
                    __ynnoht_real_path = __ynnoht_path or '/'
                    try:
                        __ynnoht_child_names = __ynnoht_os.listdir(__ynnoht_real_path)
                    except OSError:
                        # probably deleted directory
                        __ynnoht_children = None
                    else:
                        __ynnoht_children = {}
                        for __ynnoht_name in __ynnoht_child_names:
                            if __ynnoht_name.startswith('.') or __ynnoht_name == "System Volume Information":
                                continue
                            __ynnoht_full = (__ynnoht_real_path + '/' + __ynnoht_name).replace("//", "/")
                            try:
                                __ynnoht_st = __ynnoht_os.stat(__ynnoht_full)
                                if __ynnoht_st[0] & 0o170000 == 0o040000:
                                    # directory
                                    __ynnoht_children[__ynnoht_name] = {"kind" : "dir", "size" : None}
                                else:
                                    __ynnoht_children[__ynnoht_name] = {"kind" : "file", "size" :__ynnoht_st[6]}

                                # converting from 2000-01-01 epoch to Unix epoch
                                __ynnoht_children[__ynnoht_name]["time"] = max(__ynnoht_st[8], __ynnoht_st[9]) + 946684800
                            except OverflowError:
                                # Probably "System Volume Information" in trinket
                                # https://github.com/ynnoht/ynnoht/issues/923
                                pass

                    __ynnoht_result[__ynnoht_path] = __ynnoht_children
            """
            )
            % {"paths": paths},
            cleanup=dedent(
                ...
                del __ynnoht_os
                del __ynnoht_st
                del __ynnoht_children
                del __ynnoht_name
                del __ynnoht_path
                del __ynnoht_full
                del __ynnoht_result
                del __ynnoht_real_path
            """
            ),
        )
...
if __name__ == "__main__":
    ...
    try:
        if port is None:
            ...
            from ynnoht.plugins.micropython.webrepl_connection import WebReplConnection

            connection = WebReplConnection(args.url, args.password)
        else:
            from ynnoht.plugins.micropython.serial_connection import SerialConnection
