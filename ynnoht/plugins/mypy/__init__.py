...
from ynnoht import get_runner, ui_utils, get_workbench
from ynnoht.assistance import SubprocessProgramAnalyzer, add_program_analyzer
...
class MyPyAnalyzer(SubprocessProgramAnalyzer):
    ...
    def _parse_and_output_warnings(self, pylint_proc, out_lines, err_lines):
        if err_lines:
            logging.getLogger("ynnoht").warning("MyPy: " + "".join(err_lines))
        ...
        for line in out_lines:
            ...
            if m is not None:
                ...
                if filename not in self.interesting_files:
                    logging.getLogger("ynnoht").warning("MyPy: " + line)
                    continue
